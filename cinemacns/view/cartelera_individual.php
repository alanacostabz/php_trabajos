...
if (!isset($_GET['id']) || $_GET['id'] == '') {
  ...
  $movies_set = getFunctionsByMovie($id);
  $m_set = getMovieByID($id);
  $m = mysqli_fetch_assoc($m_set);
  if (empty($m)) {
    echo '<script type="text/javascript">
...
';
          ...
          if ($banderilla == 0) {
            echo '<h3>No hay funciones disponibles actualmente para esta pelicula.</h3>';
          }
          ?>
